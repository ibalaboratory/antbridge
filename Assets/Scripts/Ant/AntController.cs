...
// アリの状態を表す変数
// Random: ランダム行動/ PheromoneSearch: フェロモン探索行動/ BackHome:
// 帰巣行動/ Bridge: 利他行動
public enum AntState { Random = 0, PheromoneSearch, BackHome, Bridge }
...
public class AntController : MonoBehaviour {
  ...
  // アリのイラストの描画
  [SerializeField]
  private SpriteRenderer antRenderer = null;
  public SpriteRenderer AntRenderer {
    get { return antRenderer; }
  }
  // アリの状態
  ...
  // Colony
  [SerializeField]
  private GameObject colony = null;
  ...
  [SerializeField]
  private PheromoneController pheromonePrefab1 = null;
  private PheromoneController PheromonePrefab1 => pheromonePrefab1;
  // FeedPheromonのPrefab
  [SerializeField]
  private PheromoneController pheromonePrefab2 = null;
  private PheromoneController PheromonePrefab2 => pheromonePrefab2;
  // フェロモンを生成するステップと現在の値
  [SerializeField]
  private int pheromoneStep = 2;
  ...
  // 周囲のフェロモンの状態を保持する変数
  [SerializeField]
  private PheromoneInfo colonyPheromoneInfo = null;
  private PheromoneInfo ColonyPheromoneInfo => colonyPheromoneInfo;
  [SerializeField]
  private PheromoneInfo feedPheromoneInfo = null;
  ...
  // デバッグ用
  [Header("debug"), Multiline(7)]
  public string debugText = "No Data";

  // アリが帰巣途中に橋を渡ったかを示す変数。
  // 0:渡ってない 1:渡っている途中 2:渡った
  public int usedBridgeGoingHome = 0;
  // アリが探索状態で橋を渡ったかを示す変数。
  // 0:渡ってない 1:渡っている途中 2:渡った
  public int usedBridgeSearching = 0;
  ...
  void Start() {
    ...
    GameObject pheromoneLimitationObject =
        GameObject.Find("PheromoneLimitation");
    pheromoneLimitation =
        pheromoneLimitationObject.GetComponent<PheromoneLimitation>();
  }
  ...
  void FixedUpdate() {
    ...
    if (CurrentTime > updateSpan) {
      ...
      // 状態の遷移
      if (State == AntState.Random &&
          FeedPheromoneInfo.Count >=
              FeedPheromoneInfo
                  .Threshold) { // ランダム探索状態のときに餌フェロモンの数が一定値を上回ったとき
        State = AntState.PheromoneSearch; // フェロモン探索状態に移行
      } else if (
          State == AntState.PheromoneSearch &&
          FeedPheromoneInfo.Count <
              FeedPheromoneInfo
                  .Threshold) { // フェロモン探索状態のときに餌フェロモンの数が一定値をした下回ったとき
        State = AntState.Random; // ランダム探索状態に移行
      }

      // 進行方向の決定
      var pos = transform.position;
      if (State == AntState.BackHome) {
        DetectFeed = false;
      }
      if (State != AntState.BackHome) {
        DetectHome = false;
      ...
      if (DetectFeed) { // 餌の方向を向く
        ...
        if (usedBridgeSearching == 0) {
          if (transform.position.z <
                  CreateBridge.a_1 * transform.position.x + CreateBridge.b_1 &&
              transform.position.z <
                  CreateBridge.a_2 * transform.position.x + CreateBridge.b_2) {
            usedBridgeSearching = 1;
          }
        } else if (usedBridgeSearching == 1) {
          if (transform.position.z >
              CreateBridge.a_2 * transform.position.x + CreateBridge.b_2) {
            usedBridgeSearching = 2;
            AntBridgeManager.usedBridgeSearchingCnt++;
          } else if (transform.position.z >
                     CreateBridge.a_1 * transform.position.x +
                         CreateBridge.b_1) {
            usedBridgeSearching = 0;
          }
        }
      } else if (State == AntState.PheromoneSearch) { // フェロモン探索状態
        transform.LookAt(pos + FeedPheromoneInfo.Direction);
        // 分散1，平均0の正規分布に従う乱数
        Z = Mathf.Sqrt(-2.0f * Mathf.Log(Random.value)) *
            Mathf.Cos(2.0f * Mathf.PI * Random.value);
        ...
        if (ColonyPheromoneInfo.Count >=
            ColonyPheromoneInfo
                .Threshold) { // フェロモンの数が一定値を上回ったら、帰巣フェロモンの方向を向く
        ...
        if (usedBridgeGoingHome == 0) {
          if (transform.position.z <
                  CreateBridge.a_1 * transform.position.x + CreateBridge.b_1 &&
              transform.position.z <
                  CreateBridge.a_2 * transform.position.x + CreateBridge.b_2) {
            usedBridgeGoingHome = 1;
          }
        } else if (usedBridgeGoingHome == 1) {
          if (transform.position.z >
              CreateBridge.a_2 * transform.position.x + CreateBridge.b_2) {
            usedBridgeGoingHome = 0;
          } else if (transform.position.z >
                     CreateBridge.a_1 * transform.position.x +
                         CreateBridge.b_1) {
            usedBridgeGoingHome = 2;
          }
        } else if (usedBridgeGoingHome == 2) {
          if (transform.position.z >
              CreateBridge.a_2 * transform.position.x + CreateBridge.b_2) {
            usedBridgeGoingHome = 0;
          } else if (transform.position.z <
                         CreateBridge.a_1 * transform.position.x +
                             CreateBridge.b_1 &&
                     transform.position.z <
                         CreateBridge.a_2 * transform.position.x +
                             CreateBridge.b_2) {
            usedBridgeGoingHome = 1;
          }
        }
      ...
      if (State == AntState.BackHome || !CheckIfIsAltruistic()) {
        ...
        if (CurrentPheromoneStep > PheromoneStep && pheromoneLimitation.flg) {
          ...
          if (State == AntState.Random ||
              State ==
                  AntState
                      .PheromoneSearch) { // ランダム探索状態またはフェロモン探索状態の時
            GeneratePheromone(
                PheromonePrefab1, E * ColonyPheromoneInfo.R,
                ColonyPheromoneInfo.Alpha); // 帰巣フェロモンを放出する
          } else if (State == AntState.BackHome) { // 帰巣状態の時
            GeneratePheromone(
                PheromonePrefab2, E * FeedPheromoneInfo.R,
                FeedPheromoneInfo.Alpha); // 餌フェロモンを放出する
          }
        }
        ...
        // 以下諸変数の更新
        AntBridgeManager.elapsedTimeAsBridge++;
        if (!experience) {
          AntBridgeManager.beenBridgeCnt++;
          experience = true;
        }
      }
  ...
  public void ResetAnt() { Destroy(gameObject, 0.0f); }
  ...
  // フェロモンを生成する関数
  private void GeneratePheromone(PheromoneController prefab, float pheromone,
                                 float alpha) {
    var controller = Instantiate(
        prefab,
        transform.position + new Vector3((Random.value - 0.5f) * 2.0f, 0.0f,
  ...
  private bool CheckIfIsAltruistic() {
    ...
    if (hypothesis == 1)
      ...
      hypoCheck = (ColonyPheromoneInfo.Count + FeedPheromoneInfo.Count >=
                   pheromoneThreshold);
    ...
    if (!isAltruistic) {
      ...
      if (hypoCheck &&
          NavMesh.FindClosestEdge(purpose_pos, out hit, NavMesh.AllAreas)) {
        isAltruistic = true;
        float eps = 1e-3f;
        if (System.Math.Abs(hit.position.x - transform.position.x) < eps ||
            System.Math.Abs(hit.position.z - transform.position.z) < eps ||
            (hit.position.x - bridgePosition.x) *
                        (hit.position.x - bridgePosition.x) +
                    (hit.position.z - bridgePosition.z) *
                        (hit.position.z - bridgePosition.z) >
        ...
        direction.Normalize();
        float angle =
            (float)System.Math.Atan((hit.position.z - transform.position.z) /
                                    (hit.position.x - transform.position.x));
        // 利他行動を管理する辞書を更新
        antBridgeManager.AddAnt(ID, hit.position, angle, direction);
        bridge_spot = hit.position +
                      new Vector3(antBridgeManager.w * direction.x / 2.0f, 0f,
                                  antBridgeManager.h * direction.z / 2.0f);
      }
      return isAltruistic;
    }
    // 現在利他行動をしているとき
  ...
  private bool CheckIfStopAltruistic() {
    if (timeCnt >=
        timeThreshold) { // 利他行動を行う時間が一定以上になると利他行動をやめる
      timeCnt = 0;
      return true;
    }
    return false;
  ...
  void OnTriggerStay(Collider collider) {
    ...
    if (collider.gameObject.layer ==
        ColonyPheromoneLayer) { // 帰巣フェロモンを検出したとき
      var v = (collider.gameObject.transform.position - transform.position);
      // 内積からフェロモンの方向を計算する
      if (v.magnitude >= 0.01f &&
          Vector3.Dot(v.normalized, transform.forward) > 0.5) {
        // フェロモンの強さ、距離に応じてベクトルを加算
        ColonyPheromoneInfo.Direction +=
            collider.gameObject.GetComponent<PheromoneController>().Pheromone *
            v / Mathf.Pow(v.magnitude + 0.1f, 1.2f);
        ColonyPheromoneInfo.Count += 1;
      }
    } else if (collider.gameObject.layer ==
               FeedPheromoneLayer) { // 餌フェロモンを検出したとき
      var v = (collider.gameObject.transform.position - transform.position);
      if (v.magnitude >= 0.01f &&
          Vector3.Dot(v.normalized, transform.forward) > 0.5) {
        FeedPheromoneInfo.Direction +=
            collider.gameObject.GetComponent<PheromoneController>().Pheromone *
            v / Mathf.Pow(v.magnitude + 0.1f, 1.2f);
    ...
    // 餌と近いかどうか
    else if (collider.gameObject.layer == FeedLayer &&
             State != AntState.BackHome) {
      DetectFeed = true;
      FeedDirection =
    ...
    // 巣と近いかどうか
    else if (collider.gameObject.layer == ColonyLayer &&
             State == AntState.BackHome) {
      DetectHome = true;
      HomeDirection =
          (collider.gameObject.transform.position - transform.position)
