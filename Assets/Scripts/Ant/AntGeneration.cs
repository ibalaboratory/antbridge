...
public class AntGeneration : MonoBehaviour {
  ...
  private AntController Ant = null;

  [SerializeField]
  private CreateBridge bridgeCreater = null;
  ...
  void FixedUpdate() {
    ...
    if (currentTime >= antMakeSpan) {
      ...
      if (countSensitive <
          maxSensitiveAntNum) { // 敏感アリの数が最大値より下回っていたら、敏感アリを生成する
        GenerateAnt(sensitivityHigh, Ant1tag);
      }
      if (countInsensitive <
          maxInsensitiveAntNum) { // 鈍感アリの数が最大値より下回っていたら、鈍感アリを生成する
  ...
  private void GenerateAnt(float sensitivity, string tag) {
    ...
    // この座標は利他行動を行う際に使用される
    ant.bridgePosition = new Vector3(bridgeCreater.centerPosition.x,
                                     bridgeCreater.centerPosition.y,
                                     bridgeCreater.centerPosition.z);
  }
}
