...
public class ObstacleGenerator : MonoBehaviour {
  ...
  void CreateObstacle() {
    if (Vector3.Distance(prevPos, nowPos) == 0.0f)
      return;
    ...
    GameObject Cylinder =
        Instantiate(ObstacleCylinder, pos3, Quaternion.identity);
    ...
    Cylinder.transform.eulerAngles = new Vector3(
        90f, Vector3.SignedAngle(Vector3.forward, pos2 - pos1, Vector3.up), 0);
    Cylinder.transform.localScale =
        new Vector3(thickness, Vector3.Distance(pos1, pos2) / 2, thickness);
  }
